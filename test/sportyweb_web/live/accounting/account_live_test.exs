defmodule SportywebWeb.AccountLiveTest do
  ...
  describe "New/Edit" do
    ...
    test "saves new account", %{conn: conn, user: user} do
      ...
      assert new_live
        |> form("#account-form", account: Map.put(@invalid_attrs, :club_id, club.id))
    ...
    test "updates account", %{conn: conn, user: user, account: account} do
      ...
      valid_attrs =
        ...
        assert edit_live
          |> form("#account-form", account: Map.put(@invalid_attrs, :club_id, account.club_id))
          |> render_change() =~ "can&#39;t be blank"
